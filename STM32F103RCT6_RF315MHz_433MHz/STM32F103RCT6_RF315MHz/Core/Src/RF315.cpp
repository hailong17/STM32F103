...
uint16_t RF315::vw_crc(uint8_t *ptr, uint8_t count)
{
    uint16_t crc = 0xffff;
...
void RF315::vw_set_tx_pin(uint8_t pin)
{
    vw_tx_pin = pin;
}

// Set the ptt pin inverted (low to transmit)
void RF315::vw_set_ptt_inverted(uint8_t inverted)
{
...
uint8_t RF315::_timer_calc(uint16_t speed, uint16_t max_ticks, uint16_t *nticks)
{
    ...
    for (prescaler=1; prescaler < 7; prescaler += 1)
    {
        ...
        float clock_time = (1.0 / (float(16000000) / float(prescalers[prescaler])));
...
void RF315::vw_setup(uint16_t speed)
{
...
void RF315::vw_rx_stop()
{
    vw_rx_enabled = false;
}

// Return true if the transmitter is active
uint8_t RF315::vx_tx_active()
{
...
uint8_t RF315::vw_wait_rx_max(unsigned long milliseconds)
{
...
uint8_t RF315::vw_get_message(uint8_t* buf, uint8_t* len)
{
    uint8_t rxlen;
    // Message available?
